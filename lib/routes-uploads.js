...
export function registerUploadRoutes({ router, cfg, files, log }) {
  ...
  let reserved = 0;             // bytes promised to open sessions, counted against MAX_STORAGE_MB
  ...
  router.add('POST', '/api/uploads', async (req, res) => {
    ...
    if (cfg.maxStorageBytes !== null && files.usedBytes() + reserved + size > cfg.maxStorageBytes) throw new HttpError(507, 'storage_full', 'Storage full');
  ...
  return { pending: () => sessions.size, reservedBytes: () => reserved, close: () => clearInterval(sweep) };
}

...
export function loadConfig(overrides = {}, env = process.env) {
  ...
  for (const o of allowedOrigins) {
    try { const u = new URL(o); if (u.origin !== o) throw new Error(); } catch { throw new ConfigError(`ALLOWED_ORIGINS entry ${JSON.stringify(o)} must be a full origin like https://shelfy.example.com`); }
  }
  ...
  const cfg = {
    ...
    appName: getd('APP_NAME').trim() || 'Shelfy',
    ...
    uploadChunkBytes: overrides.uploadChunkBytes ?? 8 * 1024 * 1024,
    warnings,
  };

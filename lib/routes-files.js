...
export function registerFileRoutes({ router, cfg, files, log }) {
  // Bytes promised to uploads that are still streaming; counted against the storage cap so two concurrent uploads
  // cannot both squeeze under the limit.
  let reservedBytes = 0;
  ...
  router.add('PUT', '/api/files', async (req, res, ctx) => {
    ...
    if (cfg.maxStorageBytes !== null && files.usedBytes() + reservedBytes + declared > cfg.maxStorageBytes) {

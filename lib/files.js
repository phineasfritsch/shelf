...
export function createFiles({ cfg, db, log }) {
  ...
  let inFlight = 0;
  let reserved = 0;   // bytes promised to in-progress uploads (both paths), counted against MAX_STORAGE_MB
  ...
  const api = {
    ...
    // Storage reservation shared by BOTH upload paths (raw PUT and chunked) so a hard MAX_STORAGE_MB
    // cap holds even when a raw and a chunked upload run at the same time.
    reserve(n) { reserved += n; },
    release(n) { reserved = Math.max(0, reserved - n); },
    reserved() { return reserved; },
    // Would accepting `n` more bytes exceed the cap right now? (null cap = unlimited.)
    wouldExceedCap(n) { return cfg.maxStorageBytes !== null && api.usedBytes() + reserved + n > cfg.maxStorageBytes; },

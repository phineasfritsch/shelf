...
Shelf is a single-user app that is meant to be exposed to the internet behind HTTPS, so security bugs matter here.
...
person's shelf, log in without the password, bypass rate limiting, or run script on the Shelf origin.
...
- Running Shelf on plain `http://` across an untrusted network (documented as unsupported)
